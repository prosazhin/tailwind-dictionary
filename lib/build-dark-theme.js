...
function buildThemeBlocks({ lightVars, darkVars4, darkVars2 }) {
  return (
    `:root {\n${lightVars}}\n` +
...
function prependV4Dark({ output, themeAliases, prefix, lightTokens, darkTokens }) {
  ...
  fs.writeFileSync(themeFilePath, buildThemeBlocks({ lightVars, darkVars4, darkVars2 }) + '\n' + themeContent);
}
...
function appendV3Dark({ output, lightTokens, darkTokens }) {
  ...
  fs.writeFileSync(`${output}/tailwind/theme.css`, buildThemeBlocks({ lightVars, darkVars4, darkVars2 }));
}
